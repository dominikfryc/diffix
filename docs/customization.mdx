...
Diffix Design System can be customized at a high level through [design tokens](/?path=/docs/customization--documentation). This gives you control over theme colors and general styling. You can use component variables to target individual components for more advanced customizations.
...
If you want to implement a theme toggle, save the preferred theme outside the current session, or auto-detect the user system preference, check out our [simple demo](https://github.com/dominikfryc/diffix-test/tree/next/demo/), which implements these features.
